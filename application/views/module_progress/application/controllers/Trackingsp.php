...
class Trackingsp extends CI_Controller {
	...
	{
	$data['Shipment_id']=$cn;	
	$this->load->view('module_trackingsp/HomeView',$data); 	
	}
	...
	$is_file 		        = $shipment_data[0]['is_file'];
	$file_path		        = $shipment_data[0]['file_path'];
	//----Get Values-------End--	
	//-------Load TabMenu----End--
	...
	if($is_file==1){
	echo ("<a href='https://delex.pk/ops_delex/assets/cn_files/".$file_path ."' target='_blank' id='fixedbuttons' class='btn btn-primary'>View Attachments</a>");
	}
