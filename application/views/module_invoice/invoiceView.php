...
footer: 'true',
title:"Invoices List",
message:"T.M. Cargo <br> System Developer M.Saim <br>Date:<?php echo ''.date('Y-m-d'); ?> <br>  <br>Invoices List<br>"
}
]       
});
...
<div class="table-responsive m-t-10">
 <table class="table table-bordered" id="myTable">
  ...
  echo("<td><a href='".base_url()."Invoice/invoice_payment_view/".$rows->invoice_id."' class='btn btn-xs btn-info'>Deposit</a></td>");
  } else { echo("<td>".$rows->payment_tid."</td>");} 
  //--------------------------
  echo("<td><center>".$rows->invoice_code." (<mark>".$rows->invoice_cn."</mark>)</center></td>");
  ...
  echo("<td><center>".number_format($rows->invoice_osa_sd_total)."</center></td>");
  //--------------------------
  if($rows->invoice_permission=="Yes"){
  ...
  //--------------------------
  echo("<td><center>".$rows->fuel_surcharge."</center></td>");
  if($rows->other_amount==0){echo("<td>".$rows->other_amount."</td>");}
  else {echo("<td><b>".$rows->other_name."</b> ".$rows->other_amount."</td>");}
  //--------------------------
  echo("<td><center>".$rows->discount_amount."</center></td>");
    if($rows->invoice_permission=="Yes"){
  echo("<td class='bg-info text-white'>".number_format($rows->other_amount + $rows->invoice_gst + $rows->invoice_sc + $rows->invoice_osa_sd_total + $rows->fuel_surcharge -($rows->discount_amount) )."</td>");
    } else {
  echo("<td class='bg-info text-white'>".number_format($rows->other_amount + $rows->invoice_sc + $rows->invoice_osa_sd_total +  $rows->fuel_surcharge -($rows->discount_amount) )."</td>");
    }
