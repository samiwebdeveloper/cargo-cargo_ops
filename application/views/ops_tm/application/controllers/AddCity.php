...
class AddCity extends CI_Controller
{
	...
	public function ajaxview()
	{
		...
		$data = $this->AddCityModel->display_records_with_minimize($pagelimit);
		$i = 1;
		foreach ($data as $row) {
			...
			if ($row->is_enable) { ?>
				...
				if ($row->is_enable) { ?>
					...
					<?php } else { ?>
						<?php echo '<td><a class="btn btn-primary btn-xs" style="text-decoration: none;background: #28a745;color: white;border-radius: 5px;border: 1px solid #28a745;padding: 2px city_zonepx;" href="adduser/status/1/?id=<?= ' . $userid . ' ?>">Enable</a></td>'; ?>
				<?php }
