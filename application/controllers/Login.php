...
//    header('Location: https://cargo.tmcargoexpress.com/cargo/ops_tm');    
   header('Location: http://localhost:88/cargo/ops_tm');    
  }*/
		$data['error'] = "N";
		$this->load->view('loginView', $data);
	}
			...
			if (!empty($user_data)) {
				foreach ($user_data as $rows) {
					...
					if ($userpassword == md5($password)) {
						...
						if ($_SESSION['user_id'] == "14" or $_SESSION['user_id'] == "20" or $_SESSION['user_id'] == "76" ) {
							redirect("MainDashbord/main_home");
						} else {
							redirect('Home');
						}
					} else {
