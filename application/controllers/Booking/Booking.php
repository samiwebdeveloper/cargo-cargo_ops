...
class Booking extends CI_Controller
{
	...
	public function edit_booking_view()
	{
		...
		if ($_SESSION['origin_id']	== 130) {
			$data['customer_data'] = $this->Commonmodel->Get_record_by_double_condition('saimtech_customer', 'customer_city', 130, 'is_enable', 1);
		} else {
	...
	public function get_cong_detail()
	{
		...
		if ($customer != "" && $destination != "" && $name != "") {
			$customer_data = $this->Commonmodel->Get_record_by_condition_array('saimtech_customer', 'customer_id', $customer);
	...
	public function add_shipment()
	{
		...
		if ($m_Cn_check < 1) {
			...
			if (
				...
				$message = "<div class='pgn push-on-sidebar-open pgn-bar'><div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'><span aria-hidden='true'>×</span><span class='sr-only'>Close</span></button><strong>Missing Error !</strong> Something is missing please try again.</div></div>";
			}
		} else {
			$message = "<div class='pgn push-on-sidebar-open pgn-bar'><div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'><span aria-hidden='true'>×</span><span class='sr-only'>Close</span></button><strong>Duplicate CN Error !</strong> HAZA BAAZ AA</div></div>";
		}
		//=========================================================END
		echo $message;
	}
	...
	public function all_delete_temp_ls_by_cn()
	{
		...
		if ($_SESSION['user_power'] != "SUBAGENT") {
			...
			$data = array('temp_ls' 	 => 0);
			$this->Commonmodel->Update_triple_record('saimtech_order', 'customer_id', $cid, 'created_by', $uid, 'order_status', 'Order', $data);
			echo $count = $this->Commonmodel->Duplicate_triple_check('saimtech_order', 'customer_id', $cid, 'temp_ls', 1, 'created_by', $uid);
		}
	}
