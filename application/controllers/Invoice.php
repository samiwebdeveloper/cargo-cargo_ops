...
class Invoice extends CI_Controller {
	...
	$data['customer_data']=$this->Invoicemodel->Get_Invoice_Active_Customer();
	...
	$customer_data=$this->Invoicemodel->Get_CN_BY_Customer_ID_And_Date($customer,$invoice_date,$invoice_date_f);
	if(!empty($customer_data)){$i=0;
	foreach($customer_data as $rows){ $i=$i+1;
    ...
    echo("<td>".round($rows->order_osa_sd_total)."</td>");
	...
	if(strlen($code)==1){ $precode=$prefix."0000".$code;} 
	else if(strlen($code)==2){ $precode=$prefix."000".$code;} 
	else if(strlen($code)==3){ $precode=$prefix."00".$code;} 
	else if(strlen($code)==4){ $precode=$prefix."0".$code;} 
	else if(strlen($code)==5){ $precode=$prefix.$code;}
	return $precode;
	}
	...
	redirect('Invoice');} 
	else { echo("<p class='alert alert-dnager'>Something is missing please try again.</p>");  }
