...
class Direct extends CI_Controller
{
	...
	public function direct_process()
	{
		...
		if ($order_date < $delivery_date) {
			if (($order_status == "Refused" || $order_status == "CNA" || $order_status == "NSA" || $order_status == "OSA" || $order_status == "ICA" || $order_status == "HIO"  || $order_status == "Return" || $order_status == "RTO") &&  $is_final != 1 && $is_invoice != 1) {
				$this->db->trans_start();
				//Update Order
				$data = array(
					'order_status' 		 => $order_status,
				...
				if ($order_status == 'HIO') {
					...
					$order_message = 'The shipment has Returned to the Origin ITIF';
				...
				<strong>Alert!</strong> This CN is Not Eligible For IT Controller.
		...
		if ($cn != "" && $date != "" && $rider != ""  && $origin != "" && $receive_by) {
			...
			if ($order_date < $delivery_date) {
				if ($order_status == "Deliverd" && $is_final != 1 && $order_status != "Arrival") {
					...
					$city_data = $this->Deliverymodel->Get_City_Detail_By_id($origin);
					$origin_name = $city_data[0]['city_name'];
					$order_message = 'Shipment has been Delivered CS (' . $receive_by . ')';
					$data = array(
						'order_id' 			=> $order_id,
					...
					$city_data = $this->Deliverymodel->Get_City_Detail_By_id($origin);
					$origin_name = $city_data[0]['city_name'];
					$order_message = 'Shipment has been RTS CS (' . $receive_by . ')';
					$data = array(
						'order_id' 			=> $order_id,
						'order_event' 		 	=> 'RTS',
						'order_location' 	 	=> $origin,
					...
					$order_message = '';
					if ($order_status == 'HIO') {
						$order_message = 'Shipment hold in operation CS';
					} else if ($order_status == 'NSA') {
						$order_message = 'None Service Area CS';
					} else if ($order_status == 'OSA') {
						$order_message = 'Out Of Service Area CS';
					} else if ($order_status == 'CNA') {
						$order_message = 'Consignee Not Available CS';
					} else if ($order_status == 'ICA') {
						$order_message = 'Incomplete Address CS';
					} else if ($order_status == 'Refused') {
						$order_message = 'Shipment Refused By Consignee CS';
					} else if ($order_status == 'Return') {
						$order_message = 'Shipment Return CS';
					}
