...
class Deliverymodel extends CI_Model
{
	...
	{
	 $query = "SELECT saimtech_order.* , saimtech_customer.customer_name FROM saimtech_order
	 INNER JOIN saimtech_customer ON saimtech_order.customer_id=saimtech_customer.customer_id WHERE  (`order_code`='$cn' OR `manual_cn`='$cn') ";
		$res = $this->db->query($query);
		return $res->result_array();
	}
	public function Get_Dc_Status()
	{
	 $query = " SELECT dc_status.*,saimtech_customer.customer_name from dc_status  inner join saimtech_customer on dc_status.customer_id=saimtech_customer.customer_id";
		$res = $this->db->query($query);
		return $res->result();
	}
...
WHERE (`order_code`=? or manual_cn=?) ";
	...
	public function Get_Order_Last_Status_Id($id)
	{
		$query = "SELECT * FROM `saimtech_order_tracking` where order_id = ? order by order_tracking_id DESC LIMIT 1,1";
		$res = $this->db->query($query, array($id));
		return $res->result_array();
	}
	//-------------Remove------------END
